...
final class Version20250306122754 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $schemaName = $this->connection->fetchOne("SELECT current_schema");
        foreach($this->tables_to_migrate as $table) {
            ...
            if ($columnCheck !== 'YES') {
                $this->addSql('DROP SEQUENCE ' . $schemaName .'.' . $table . '_id_seq CASCADE');
    ...
    public function down(Schema $schema): void
    {
        ...
        foreach($this->tables_to_migrate as $table) {
            ...
            $this->addSql('DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = \'' . $table . '_id_seq\') THEN CREATE SEQUENCE ' . $schemaName . '.' . $table . '_id_seq INCREMENT BY 1 MINVALUE 1 START 1; END IF; END $$;');

            $this->addSql('ALTER TABLE '. $table . ' ALTER COLUMN id SET DEFAULT nextval(\'' . $schemaName . '.' . $table . '_id_seq\')');

            $this->addSql('SELECT setval(\'' . $schemaName . '.' . $table . '_id_seq\', (SELECT COALESCE(MAX(id), 1)+1 FROM ' . $table . '), false);');
        }
