...
test.describe("drawerPermissions", () => {
  ...
  test.describe("the personal group", () => {
    test.beforeAll(async ({ browser }) => {
      await ensureNonManagerUser(browser);
    });
    ...
    test("appears flagged after creating a drawer, and takes members", async ({
      page,
    }) => {
      // creating a drawer auto-creates (or reuses) the owner's personal group
      const drawerRes = await page.request.post(
        `${baseURL()}/drawers/addDrawer/true`,
        { form: { drawerTitle: uniqueLabel("Test Drawer") } }
      );
      ...
      expect(listRes.status()).toBe(200);
      const { groups } = (await listRes.json()) as {
        groups: (Group & { is_personal: boolean })[];
      };
      const personal = groups.find((g) => g.is_personal);
      expect(personal).toBeTruthy();
      expect(personal?.type).toBe("User");

      // A rename would also prove editability, but personal-group
      // detection matches on the label and drawer tables are never
      // reset, so a rename would unflag the group for every later run.
      const guest = await findUserByUsername(page, NON_MANAGER.username);
      const add = await page.request.post(
        `${baseURL()}/drawerPermissions/groups/${personal?.id}/members`,
        { form: { localUserId: String(guest.localUserId) } }
      );
      expect(add.status()).toBe(201);
