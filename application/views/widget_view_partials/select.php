...
<ul>
	...
	<?if(array_keys($widgetModel->parsedFieldData["selectGroup"]) !== range(0, count($widgetModel->parsedFieldData["selectGroup"]) - 1)):?>
		<li><?=$widgetModel->getClickToSearch()?"<A href=\"".instance_url("/search/querySearch/". rawurlencode($widgetModel->parsedFieldData["selectGroup"][$fieldContent->fieldContents])) ."\">".$widgetModel->parsedFieldData["selectGroup"][$fieldContent->fieldContents]."</a>":$widgetModel->parsedFieldData["selectGroup"][$fieldContent->fieldContents];?></li>
	<?else:?>
