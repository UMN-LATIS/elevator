...
<?else:?>
	...
	<div class="fullscreenImageContainer">
		...
		<?if(count($fileContainers)>0):?>
			<img class="img-responsive embedImage imageContent"
				src="<?=stripHTTP(array_values($fileContainers)[0]->getProtectedURLForFile())?>"
				alt="<?=($widgetObject && $widgetObject->fileDescription )?htmlspecialchars($widgetObject->fileDescription, ENT_COMPAT):null?>" />
		<?endif?>
		</div>
...
<script>
	...
	$(document).on("click", ".canFullscreen", function () {
		if ($.fullscreen.isNativelySupported()) {
			$(".imageContainer").first().fullscreen({
				"toggleClass": "imageFullscreen"
			});
		}
	});
