...
class Upload_contents extends Widget_contents_base {
	...
	public function getAsArray($serializeNestedObjects=false) {
		if(isset($this->fileHandler)) {
			...
			return array_merge($dateData, ["fileId"=>$this->fileHandler->getObjectId(), "fileDescription"=>$this->fileDescription, "fileType"=>$this->fileHandler->sourceFile->getType(), "searchData"=>$this->searchData, "loc"=>$location, "sidecars"=>$this->sidecars, "isPrimary"=>$this->isPrimary]);
		}
		else {
			return array();
		}
