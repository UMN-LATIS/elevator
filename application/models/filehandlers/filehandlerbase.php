...
class FileHandlerBase extends CI_Model {
	...
	function findDeletedItems() {
		...
		return $this->doctrine->getRepository("Entity\FileHandler")->findBy(["deleted"=>true]);
	}

	function deleteSource($serial=null,$mfa=null) {

		if($mfa && $this->sourceFile->deleteFile($serial, $mfa)) {
			$asset = $this->doctrine->em->getRepository("Entity\FileHandler")->findBy(["fileObjectId"=>$this->getObjectId()]);
