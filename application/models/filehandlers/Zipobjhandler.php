...
class ZipObjHandler extends ZipHandler {
	...
	public function createDerivative($args) {
		...
		$meshlabCommandLine =  $this->config->item("meshlabPath") . " obj_to_ply " . escapeshellarg($objFile) . " " . $derivativeContainer->getPathToLocalFile() . ".ply";
		...
		foreach ($glbDerivativeSets as $label => $scale) {
			...
			// we change dir inside docker so we have to pass in two args
			$blenderCommandLine =  $this->config->item("blenderBinary") . "  -P /root/glb.py -- " . escapeshellarg($objFile) . " " . $scale . " glb";
		...
		// we change dir inside docker so we have to pass in two args
		$blenderCommandLine =  $this->config->item("blenderBinary") . "  -P /root/glb.py -- " . escapeshellarg($objFile) . " " . $scale . " usdz";
		exec($blenderCommandLine . " 2>/dev/null");
		if (!file_exists(str_replace(".obj", "_output.usdz", $objFile))) {
			// failed to process with the texture, let's try without.
			echo "Failed to generate USDZ for\n";
			$this->logging->processingInfo("createDerivative", "objHandler", "Failed to generate USDZ", $this->getObjectId(), 0);
		}
