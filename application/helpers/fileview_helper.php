...
function renderFileMenu($menuArray) {
	...
	if(isset($menuArray['fileInfo'])) {
		$outputText .= '<span class="glyphicon glyphicon-info-sign infoPopover infoToolip" aria-haspopup="true" aria-expanded="false" role="button" data-placement="bottom" data-toggle="popover" title="File Info" data-html=true data-content=\'<ul class="list-group">';
	...
	if(isset($menuArray['download'])) {
		...
		$outputText .= ' <span class="glyphicon glyphicon-download infoPopover infoToolip" aria-haspopup="true" role="button" aria-expanded="false" data-placement="bottom" data-toggle="popover" title="Download" data-html="true" data-content=\'<ul>';
	...
	if(isset($menuArray['embed'])) {
		$outputText .= '  <span class="glyphicon glyphicon-share infoPopover infoToolip" aria-haspopup="true" aria-expanded="false" role="button" data-placement="bottom" data-toggle="popover" title="Share" data-html="true" data-content=\'<ul>';
	...
	$outputText .= ' <span class="glyphicon glyphicon-resize-full triggerLightbox infoTooltip" title="Open in Lightbox"></span>';

	if(isset($menuArray['zoom'])) {
		$outputText .= '<span></span><input type="range" class="zoom-range" min=20 value=20>
			<span class="canFullscreen glyphicon glyphicon-fullscreen infoTooltip" data-toggle="popover" title="Fullscreen" data-toggle="tooltip" title="Fullscreen"></span>';
	}
