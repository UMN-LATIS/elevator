...
function loadStudy(studyViewer, studyJSON) {
        ...
        studyJSON.seriesList.forEach(function(series) {
            var stack = {
            ...
            if (series.numberOfFrames !== undefined) {
                ...
                for (var i = 0; i < numberOfFrames; i++) {
                    ...
                    console.log(imageId);
                    if (imageId.substr(0, 4) !== 'http' && imageId.substr(0, 5) !== 'https' && imageId.substr(0, 8) !== 'dicomweb') {
                        imageId = "dicomweb://localhost/dicom/" + imageId;
                    }
                ...
                series.instanceList.forEach(function(image) {
                    ...
                    if (image.imageId.substr(0, 4) !== 'http' && image.imageId.substr(0, 5) !== 'https' && image.imageId.substr(0, 8) !== 'dicomweb') {
                        imageId = "dicomweb://localhost/dicom/" + image.imageId;
                    }
                    stack.imageIds.push(imageId);
                });
            }
        ...
        viewportWrapper.style.height = (parentDiv.style.height - 100) + "px";
        ...
        cornerstone.loadAndCacheImage(imageId).then(function(image) {
            ...
            setupViewportOverlays(element, studyJSON);
            ...
            function resizeStudyViewer() {
                ...
                $(parentDiv).width(width - 100);
                viewportWrapper.style.width = ($(parentDiv).width() - 10) + "px";

                viewportWrapper.style.height = ($(parentDiv).height() - 150) + "px";
                cornerstone.resize(element, true);
            }

            $(window).on("fscreenchange", function() {
                resizeStudyViewer();
            });
